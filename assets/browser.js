...
var canvas, d3, game, height, info, legend, move, populate, run, running, simulation, space, ui, width;
...
canvas = d3.select("#space").append("svg:svg").attr("height", height).attr("width", width);

info = d3.select("#controls").append("svg:svg").attr("height", height);

legend = simulation.strategies;

info.selectAll("circle").data(legend).enter().append("circle").attr("cx", function(d, i) {
  return 50;
}).attr("cy", function(d, i) {
  return (i * 50) + 25;
}).style("fill", function(d) {
  return d.color;
}).style("opacity", 0.5).attr("r", 8).attr("title", function(d) {
  return d.name;
});

info.selectAll("text").data(legend).enter().append("text").text(function(d) {
  return d.name;
}).attr("x", function(d, i) {
  return 75;
}).attr("y", function(d, i) {
...
populate = function() {
  ...
  return canvas.selectAll("circle").data(space).enter().append("circle").style("fill", function(d) {
...
},{"./assets/d3.min.js":1,"./assets/jquery-ui-1.11.2.custom/jquery-ui.js":2,"./simulation.coffee.md":4}],4:[function(require,module,exports){
...
walk = function(agent) {
  ...
  if (agent.x > agent.space.width) {
    agent.x = agent.space.width - xRand / 2;
  }
